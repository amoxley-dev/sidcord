...
function DmMessageBody(props) {
  const [body, setBody] = useState(props.dmMessage.body)
  const [edit, setEdit] = useState(false)

  useEffect(() => {
    if (edit) {
      const messageContainer= document.querySelector(`#dmMessage-${props.dmMessage.id}`);
      messageContainer.classList.add("editing");

      const close = (e) => {
        if (e.keyCode === 27) {
          setEdit(!edit)
          setBody(props.dmMessage.body)
        }
      };
  ...
  const messageOptions = () => {
    if (props.currentUserId === props.dmMessage.user.id) {
      return (
  ...
  const handleSubmit = (e) => {
    ...
    let dmMessage = props.dmMessage;
    dmMessage.body = body;
    props.updateDmMessage(dmMessage)
    setEdit(!edit)
  }
